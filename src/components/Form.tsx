    ...
    return (
        <div className="relative w-full h-full">
            <form onSubmit={handleSubmit(submitForm)} className="absolute top-0 left-0 sm:top-1/2 sm:transform sm:-translate-y-1/2
                ...
                <StringInput title="CARDHOLDER NAME" name="nameCard" register={register} formState={formState} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updatefield("nameCard", e.target.value)} />
                <StringInput title="CARD NUMBER" name="numberCard" register={register} formState={formState} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updatefield("numberCard", e.target.value)} />
                <div className="flex flex-row items-center justify-between gap-2 sm:gap-4">
                    ...
                    <StringInput title="CVC" name="cvc" register={register} formState={formState} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updatefield("cvc", e.target.value)} />
                </div>
